...
tikz("experiment1.tex", width = 6.3, height = 3, standAlone = F)
...
plot(NA, xlab="k", ylab="Avg time per document [$\\mu$secs]",
...
legend("topright",
        legend=c("$K^2$-treap retrieved","mixed","RMQC + CSA retrieved"),
        fill=c(k2_treap_col, mixed_col, rmq_csa_col),
        bty="n", cex=0.9
        )
...
for ( i in 1:nrow(raw2[["0"]][-1,]) ){
    ...
    lines( rep(i,2)-0.15,
           c(0.001, avg_multi_occ_time[i]),
           lwd=5, col=k2_treap_col, lend=1 ) 
    lines( rep(i,2),
           c(0.001, time_per_doc[i]),
           lwd=5, col=mixed_col, lend=1 ) 
    lines( rep(i,2)+0.15,
           c(0.001, avg_single_occ_time[i]),
