...
do
    for idx in $INDEXES
    do
        $SURF_PATH/build_turpin/surf_daemon-$idx -c $col -p $PORT &
        for k in 10 100 1000 
        do
            $SURF_PATH/build_turpin/surf_query -h localhost:$PORT -q $SURF_PATH/queries/trec2005-efficiency-1000.qry -k $k -r 1 -p >> $EXP_DIR/nodes_evaluated_2005.csv
            $SURF_PATH/build_turpin/surf_query -h localhost:$PORT -q $SURF_PATH/queries/trec2006-efficiency-1000.qry -k $k -r 1 -p >> $EXP_DIR/nodes_evaluated_2006.csv
		done
        # shut down daemon
